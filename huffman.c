...
bool huffman_encode_byte(node_t* root, unsigned char byte, bitstream_t* acc)
{
    state_t initial_state = { .state = ST_VAL, .node = root };

    array_t* history = array_create(sizeof(state_t), 8);
    array_append(history, &initial_state);
    ...
    while(true)
    {
        ...
        if(curr_state.state == ST_VAL)
        {
            ...
            {
                curr_state.state = ST_RIGHT;
                array_append(history, &curr_state);
            }
        ...
        else if(curr_state.state == ST_RIGHT)
        {
            ...
            curr_state.state = ST_LEFT;
            array_append(history, &curr_state);

            curr_state.state = ST_VAL;
            curr_state.node = curr_state.node->right;
            array_append(history, &curr_state);
        }
        else if(curr_state.state == ST_LEFT)
        {
            ...
            curr_state.state = ST_DONE;
            array_append(history, &curr_state);

            curr_state.state = ST_VAL;
            curr_state.node = curr_state.node->left;
            array_append(history, &curr_state);
        }
        else
        {
